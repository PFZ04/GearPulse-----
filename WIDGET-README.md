...
当前登录任务运行版本 **V1.3.7**（产品版本 `1.3.7`，Windows 文件版本 `1.3.7.0`），位于 `publish/win-x64/GearPulse.exe`。替换前的 1.3.6 已备份为 `publish/GearPulse-1.3.6-before-gamesir.exe`；独立候选包和安装包分别位于 `publish/gamesir-candidate/GearPulse.exe` 与 `publish/GearPulse-1.3.7-Setup.exe`。已有功能包括通用雷蛇设备发现、外观设置、通用 ATK／VXE／VGN 设备支持和简体中文、English、繁體中文界面。用户已确认 Viper V4 Pro 能显示型号和电量，但充电状态不可读；其他雷蛇型号仍需在对应实机验证。

安装包可直接在 Windows x64 电脑上运行；打包本身不会修改当前本机安装。安装包复用原有 GearPulse 安装身份，覆盖安装时保留已有设置。
...
手柄行支持已连接的物理 USB、蓝牙和 2.4G 游戏手柄，排除只有虚拟设备节点的模拟手柄；同一物理设备的多个接口按容器合并。蓝牙已配对但没有当前游戏输入接口的设备不显示，以免沿用旧电量。Xbox 蓝牙手柄优先显示 Windows 报告的百分比；启明星 2 的 `3537:1098` 2.4G 接收器使用已验证的专用 HID 设备信息查询读取真实百分比，不依赖 GameSir Connect。查询失败、休眠或关机后不沿用旧电量。启明星 2 USB 有线 `3537:100F` 可识别型号，但它的设备信息电量字段为 `255`（充电哨兵值），不作为 100%；蓝牙 Android HID 模式 `3537:100E` 没有已验证的百分比接口。蓝牙 Windows 模式会模拟 Xbox 手柄，Windows 不提供可安全区分该设备身份及读取真实百分比的接口。其他手柄使用 Windows 的电量值；只有唯一物理 XInput 手柄与唯一 XInput 插槽能对应时才采用 XInput 档位，显示“空／低／中／满”，不把档位换算为百分比。没有可靠来源时保留手柄行并显示“电量暂不可用”。飞智八爪鱼 4 在 Windows 可识别时按通用手柄显示；其专有电量协议及各连接模式仍待实机验证。
