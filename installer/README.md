...
`publish\GearPulse-1.3.8-Setup.exe` 是 Windows x64 的当前用户安装包；目标电脑无需 .NET SDK、PowerShell 7 或 Inno Setup。安装到 `%LOCALAPPDATA%\Programs\GearPulse`，创建开始菜单入口和卸载记录。安装向导支持简体中文、繁體中文和英文，默认启用登录时启动并在安装结束运行 GearPulse。取消登录启动时，安装程序会注册一个禁用的任务，因此托盘开关仍可使用。

安装包由 `publish\valkyrie-final\GearPulse.exe` 构建；构建过程不会运行安装程序。
...
默认命令打包 `publish\win-x64\GearPulse.exe`。打包 V1.3.8 候选版时运行：
